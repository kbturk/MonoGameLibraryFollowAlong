...
public class Tilemap
{
    ...
    /// <summary>
    /// Gets the Tilewidth, in pixels, each tile is drawn at.
    /// </summary>
    public float TileWidth =>  _tileset.TileWidth * Scale.X;

    /// <summary>
    /// Gets the Tileheight, in pixels, each tile is drawn at.
    /// </summary>
    ...
    ///<summary>
    ///Creates a new tilemap based on a tilemap xml configuration file that contains, among other things,
    ///a path to the image that is loaded into the content manager.
    ///So, a file that is linked in the content manager can be used to create a textureRegion
    ///which can be used to create a tileSet, which can be used to create a tileMap object.
    ///</summary>
    ///<param name = "content">The content manager used to Load the texture from the tileset.</param>
    ///<param name = "filename">The path to the xml file, relative to the content root director.</param>
    ///<returns> The tilemap created by this menthod.</returns>
    public static Tilemap FromFile(ContentManager content, string filename)
    {
        ...
        using (Stream stream = TitleContainer.OpenStream(filePath))
        {
            using (XmlReader reader = XmlReader.Create(stream))
            {
                ...
                // The <Tileset> element contains the information about the tilesest 
                // used by the tilemap.
                //
                // Example
                // <Tileset region="0 0 100 100" titleWidth="10" titleHeight="10">contentPath</Tileset>
                //
                ...
                //
                // the tileWidth/tileHeight attributes specify the width and
                ...
                // To capture this map, first split on the newline to get row tile data.
                string[] rows = tilesElement.Value.Trim().Split('\n', StringSplitOptions.RemoveEmptyEntries);

                // Split on the space character and count to get the number of columns:
                ...
                // process each row to generate an index array of tile numbers (int)
                for (int row = 0; row < rows.Length; row++)
                {
                    // split
                    string[] columns = rows[row].Trim().Split(" ", StringSplitOptions.RemoveEmptyEntries);

                    // Process each column (aka entry) of current row:
                    for (int column = 0; column < columnCount; column++)
                    {
                        int tilesetIndex = int.Parse(columns[column]);

                        // add the region to the tilemap
